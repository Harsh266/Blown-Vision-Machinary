...
<<<<<<< HEAD
              ...
              <div key={i} className={`flex items-center justify-between bg-white border border-[#BFC9C3]/30 rounded-2xl px-5 py-5 attach-hover reveal reveal-up d-${(i % 6) + 1}`}>
                <div className="flex items-center gap-4">
                  <div className="w-11 h-11 rounded-xl bg-[#E7E8E9] flex items-center justify-center shrink-0">
                    <Icon size={20} className="text-[#003429]" />
                  </div>
                  <div>
                    <p className="text-sm sm:text-base font-bold text-[#003429]">{att.title}</p>
                    <p className="text-xs sm:text-sm text-[#404945]">{att.desc}</p>
                  </div>
                </div>
      ...
      <section className="py-16 px-4 sm:px-6 md:px-10 lg:px-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {[
            { icon: ShoppingBag, title: "Shopping Bags", desc: "High-tensile strength carrier solutions for retail environments." },
            { icon: ShoppingCart, title: "Grocery Bags", desc: "Food-grade, lightweight packaging for daily supermarket needs." },
            { icon: Trash2, title: "Trash Bags", desc: "Heavy-duty puncture-resistant liners for industrial waste management." },
            { icon: Layers, title: "Shrink Film", desc: "Thermal-wrap protection for securing bulk goods during transport." },
            { icon: AlignLeft, title: "Lamination Film", desc: "Premium protective barriers for high-quality flexible packaging." },
            { icon: Package, title: "Liner Bags", desc: "Durable inner liners for industrial and commercial packaging." },
          ].map((app, i) => {
            const Icon = app.icon;
            return (
              <div key={i} className={`bg-white rounded-2xl p-6 sm:p-8 flex flex-col items-center text-center card-hover reveal reveal-scale d-${i + 1}`}>
                <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-[24px] bg-[#003429]/5 flex items-center justify-center mb-6">
                  <Icon size={26} className="text-[#004D3D]" />
                </div>
